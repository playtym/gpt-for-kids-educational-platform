...
export const GlobalOnboarding: React.FC<GlobalOnboardingProps> = ({
  ...
  const handleComplete = async () => {
    ...
    try {
      // Create user in context (this automatically sets as current user)
      const newUser = createUser(formData.name, age, formData.grade);
      ...
      console.log('Set educational context');
      
      // Mark onboarding as complete for this user
      completeOnboarding(newUser.id);
      ...
      console.log('Onboarding completed successfully for user:', newUser.id);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-lg bg-white rounded-2xl shadow-xl border-0">
        <DialogHeader className="space-y-3 pb-4">
          <DialogTitle className="flex items-center space-x-3 text-xl">
            <div className="p-2 bg-blue-50 rounded-lg">
              <StepIcon size={24} className="text-blue-600" />
            </div>
            <span className="font-semibold text-gray-800">Welcome to Plural!</span>
          </DialogTitle>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between text-sm">
              <span className="font-medium text-gray-700">Step {stepNumber} of {totalSteps}</span>
              <span className="text-gray-500">{Math.round((stepNumber / totalSteps) * 100)}% complete</span>
            </div>
          ...
          <div className="flex justify-between space-x-4 pt-4 border-t border-gray-200">
            <Button
              ...
              onClick={handleBack}
              disabled={step === 'name'}
              className="flex-1 min-h-[44px] border-gray-300 hover:border-gray-400 disabled:opacity-50"
            >
            ...
            <Button
              onClick={handleNext}
              className="flex-1 min-h-[44px] bg-blue-600 hover:bg-blue-700 text-white font-medium flex items-center justify-center space-x-2"
            >
