...
interface CreateModeProps {
  onStartCreation: (prompt: string, creationType: string, style: string) => void;
  ageGroup: string;
  userId?: string;
}

const CreateMode: React.FC<CreateModeProps> = ({ onStartCreation, ageGroup, userId }) => {
  const { 
    topics: quickTopics, 
    isLoading, 
    refreshTopics, 
  ...
  const handleTopicClick = (question: string, title: string) => {
    ...
    onStartCreation(question, title.toLowerCase(), 'magical');
  };
              ...
              <div key={topic.title} className="space-y-1">
                {topic.questions.slice(0, 2).map((question, idx) => (
                  <Button
                    ...
                    variant="ghost"
                    size="sm"
                    className="w-full justify-start text-left h-auto p-2 text-xs hover:bg-pink-50 leading-tight"
