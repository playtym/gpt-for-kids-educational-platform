...
const LearnMode: React.FC<LearnModeProps> = ({ onStartLearning, ageGroup, userId }) => {
  const { 
    topics: quickTopics, 
    isLoading, 
    refreshTopics, 
      ...
      <Card className="w-full">
        <CardContent className="p-3 sm:p-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {quickTopics.slice(0, 4).map((topic, topicIndex) => (
              <div key={`${topic.title}-${topicIndex}`} className="space-y-1">
                {topic.questions.slice(0, 2).map((question, questionIndex) => (
                  <Button
                    key={`${topicIndex}-${questionIndex}`}
                    variant="ghost"
                    size="sm"
                    className="w-full justify-start text-left h-auto p-2 text-xs hover:bg-purple-50 leading-tight"
