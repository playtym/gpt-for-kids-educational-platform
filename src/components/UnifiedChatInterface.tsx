...
interface UnifiedChatInterfaceProps {
  ...
  isMobile?: boolean; // Mobile-specific layout flag
}
...
export const UnifiedChatInterface: React.FC<UnifiedChatInterfaceProps> = ({ className = '', threadId, isMobile = false }) => {
  ...
  useEffect(() => {
    try {
      ...
      if (currentThread && Array.isArray(currentThread.messages)) {
        ...
        // Only clear messages if we have a thread ID but no thread data
        // This prevents clearing messages during thread creation
        setMessages([]);
      }
    } catch (error) {
      console.error('Error updating messages from thread:', error);
      // Only reset to safe state if we don't have existing messages
  ...
  }, [currentThreadId, getCurrentThread, toast, messages.length]);
  ...
  const processApiRequest = async (threadId: string, query: string, mode: ChatMode, effectiveAgeGroup: AgeGroup, existingUserMessage?: ChatMessage) => {
    ...
    // Ensure contextMessages is always defined, even if empty
    const contextMessages = [
      {
        role: 'assistant' as const,
        content: `CONVERSATION MEMORY:\n${memoryContext?.contextPrompt || 'This is the beginning of a new conversation.'}`,
  ...
  const processLearningJourney = async (threadId: string, query: string, effectiveAgeGroup: AgeGroup, existingUserMessage?: ChatMessage) => {
    try {
      ...
      if (isNewLearningConversation) {
        ...
        setLoadingStage('Starting learning journey...');
        ...
        setLoadingStage('Processing your answer...');
        try {
          response = await agentService.processLearningAnswer(threadId, query, effectiveAgeGroup);
        } catch (error) {
          // If no active learning journey found, start a new one
          if (error instanceof Error && error.message.includes('No active learning journey')) {
            console.log('No active journey found, starting new one...');
            setLoadingStage('Starting new learning journey...');
            response = await agentService.startLearningJourney(threadId, query, effectiveAgeGroup);
          } else {
            throw error;
          }
  ...
  const handleLearningAnswer = async (threadId: string, answer: string) => {
    ...
    try {
      let response;
      try {
        response = await agentService.processLearningAnswer(threadId, answer, ageGroup || '8-10');
      } catch (error) {
        // If no active learning journey found, treat this as starting a new journey
        if (error instanceof Error && error.message.includes('No active learning journey')) {
          console.log('No active journey in handleLearningAnswer, starting new one...');
          setLoadingStage('Starting new learning journey...');
          response = await agentService.startLearningJourney(threadId, answer, ageGroup || '8-10');
        } else {
          throw error;
        }
      }
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to process your answer';
      toast({
        title: "Learning Journey Error",
        description: errorMessage.includes('No active learning journey') 
          ? "Let's start fresh! Please ask your question again to begin a new learning journey."
          : errorMessage,
        variant: "destructive"
      });
    } finally {
  ...
  return (
    <div 
      className={`${isMobile ? 'h-screen grid' : 'flex flex-col h-full'} bg-white`} 
      style={{ 
    ...
    >
      ...
      {/* Messages Area with Duolingo-inspired background */}
      <div 
        className={`${isMobile ? 'overflow-hidden' : 'flex-1 overflow-hidden'}`}
      ...
      >
        ...
        >
          <div 
            className={`space-y-4 ${isMobile ? 'p-4 pb-4' : 'p-4'}`} 
            style={{ backgroundColor: '#F7F8FC' }}
          >
      ...
      {/* Input Area - Duolingo-inspired design */}
      <div 
        className={`bg-white ${isMobile ? 'p-4 safe-area-inset-bottom' : 'p-6'}`}
        style={{ 
        ...
        <form onSubmit={handleSubmit} className={`flex items-end ${isMobile ? 'space-x-2' : 'space-x-3'}`}>
          ...
          <DropdownMenu key={`mode-selector-${currentMode}`}>
            <DropdownMenuTrigger asChild>
              <Button 
                variant="outline" 
                className={`flex items-center space-x-2 border-2 rounded-2xl transition-all duration-200 ${isMobile ? 'px-3 py-2 h-12' : 'px-4 py-2'} ${
                  messages.length > 0 
              ...
              >
                {React.createElement(modeConfig[currentMode].icon, { size: isMobile ? 16 : 18 })}
                {!isMobile && (
                  <>
            ...
            <DropdownMenuContent className="w-72 p-2">
              {messages.length > 0 ? (
                <div className="p-3 text-center text-sm text-gray-500">
                  <p className="font-medium">Mode Locked</p>
                  <p className="text-xs mt-1">Clear chat to change modes</p>
                </div>
              ) : (
          ...
          <div className="flex-1 relative">
            <Input
              ...
              className={`pr-12 bg-white border-2 focus:border-green-500 focus:ring-2 focus:ring-green-500/20 rounded-2xl resize-none transition-all duration-200 ${
                isMobile ? 'text-base py-3 h-12' : 'text-sm py-3 min-h-[44px]'
              }`}
              style={{
          ...
          <Button 
            ...
            className={`text-white rounded-2xl shadow-lg transition-all duration-200 font-bold active:scale-95 ${
              isMobile ? 'px-6 py-3 h-12' : 'px-6 py-3 min-h-[44px]'
            }`}
            style={{
              background: isLoading ? '#9CA3AF' : 'linear-gradient(135deg, #58A700 0%, #4A8B00 100%)',
              border: 'none',
              boxShadow: '0 4px 12px rgba(88, 167, 0, 0.3)'
            }}
          >
            {isLoading ? (
