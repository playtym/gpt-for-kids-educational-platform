  ...
  useEffect(() => {
    try {
      ...
      try {
        ...
        localStorage.setItem('chatThreads', JSON.stringify(threads.slice(-10))); // Keep only last 10 threads
      } catch (fallbackError) {
        console.error('Failed to save threads even after cleanup:', fallbackError);
      }
    }
  }, [threads]);
