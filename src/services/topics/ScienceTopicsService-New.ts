...
export class ScienceTopicsService extends BaseTopicsService {
  getTopicsForAge(ageGroup: AgeGroup): AgeGroupTopics {
    switch (ageGroup) {
      case '5-6':
   ...
   */
  getContextualSuggestions(topic: string, ageGroup: AgeGroup): QuickTopic[] {
    const suggestions: QuickTopic[] = [];
    const allTopics = this.getTopicsForAge(ageGroup);
